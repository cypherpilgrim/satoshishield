...
The same thing happens with his Bitcoin addresses. To spot incoming money, Carlos's wallet does not wait for a payment to arrive — it watches a block of addresses running ahead of the ones he has already used, asking a remote server about each of them so it will notice funds the moment they land. As he transacts, that block slides forward and the wallet hands the server the next batch. The server logs all of it. After a month, the surveillance company holds an inventory of the addresses Carlos's wallet has used and the ones it is about to use — supplied, in the order they were generated, by the wallet itself through its routine operation. It does not need to guess; the list arrives on its own.
...
Modern Bitcoin wallets generate a fresh address every time the user needs one. This is good privacy practice in principle — it stops anyone watching the blockchain from easily tying all of a person's transactions to a single address. The trouble is that the wallet then has to monitor all of those addresses to know when funds arrive, and it does that by asking a remote server.

It cannot do this one address at a time, because it has to be watching an address before money lands on it — and it can't know in advance which of its unused addresses a sender will pay. So the wallet watches a moving window: every address that has already received funds, plus a buffer of still-unused addresses sitting just ahead of them. That buffer — the "gap limit" — is commonly around twenty. The day Carlos installs the wallet, it already asks the server about that whole opening batch, empty addresses and all. As payments come in and the used frontier advances, the window slides forward and the wallet hands the server the next run of look-ahead addresses to keep the buffer ahead of him. The disclosure runs ahead of his spending, not behind it.

From the wallet's side this is routine bookkeeping. From the server's side it is the wallet arriving in installments — not a trickle of one address per payment, but an expanding map: the addresses Carlos has used, the addresses he is about to use, and, as the window keeps advancing over the months, effectively every address the wallet will ever watch. The same thing runs in parallel on a second, hidden track of change addresses. Before long the server holds the shape of the whole wallet.

There is a quieter detail that makes this worse than it sounds. The disclosure does not stop because Carlos changes his behavior — it stops only when he stops using that wallet. As long as he keeps the same recovery phrase, every new transaction pushes the window forward and adds addresses to the file, and switching to a Pi-hole tomorrow does not unleak yesterday's queries. And there is a sharper exposure that is easy to underestimate: the addresses a wallet generates are not random. They are all derived from a single master public key, the xpub, which is supposed to live only inside the wallet. But ordinary actions can hand it over — pasting it into a block explorer's "xpub watch" field, importing it into a portfolio tracker, installing a wallet app whose backend receives the xpub for "sync" or "discovery." Anyone holding the xpub can derive every address the wallet has ever generated and every address it ever will. Addresses leak a window at a time; an xpub leaks the wallet's entire past and future in a single keystroke.
